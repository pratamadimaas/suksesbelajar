        ...
        <form action="{{ route('admin.soal.store') }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <select name="kategori" id="kategori" class="form-select @error('kategori') is-invalid @enderror">
                    ...
                    <option value="TWK" {{ old('kategori') == 'TWK' ? 'selected' : '' }}>TWK</option>
                    <option value="TIU" {{ old('kategori') == 'TIU' ? 'selected' : '' }}>TIU</option>
                    <option value="TKP" {{ old('kategori') == 'TKP' ? 'selected' : '' }}>TKP</option>
                </select>
                @error('kategori')
            ...
            <div class="row">
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <textarea name="pilihan_{{ $option }}" id="pilihan_{{ $option }}" class="form-control @error('pilihan_' . $option) is-invalid @enderror" rows="2">{{ old('pilihan_' . $option) }}</textarea>
            ...
            <div class="mb-3">
                ...
                <select name="kunci_jawaban" id="kunci_jawaban" class="form-select @error('kunci_jawaban') is-invalid @enderror">
                    ...
                    <option value="{{ $key }}" {{ old('kunci_jawaban') == $key ? 'selected' : '' }}>{{ $key }}</option>
                    @endforeach
                </select>
                @error('kunci_jawaban')
                <div class="invalid-feedback">{{ $message }}</div>
            ...
            <div id="tkp-skor-fields" style="display: none;">
                ...
                <div class="row">
                    ...
                    <div class="col-6 col-md-4 col-lg-2 mb-3">
                        ...
                        <input type="number" name="skor_{{ $option }}" id="skor_{{ $option }}" class="form-control @error('skor_' . $option) is-invalid @enderror" value="{{ old('skor_' . $option) }}">
                        @error('skor_' . $option)
                        <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
            ...
            <div class="mb-3">
                ...
                <textarea name="pembahasan" id="pembahasan" class="form-control @error('pembahasan') is-invalid @enderror" rows="3">{{ old('pembahasan') }}</textarea>
                @error('pembahasan')
                <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
